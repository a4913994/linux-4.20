...
/**
 * DOC: Overview
 * KSM (Kernel Same-page Merging) 是一种 Linux 内核提供的虚拟内存管理机制，它可以在多个进程之间寻找并合并相同的内存页面，减少内存占用并提高系统性能。
 * 在使用 KSM 时，内核会遍历系统中所有进程的地址空间，找到相同的页面，并将它们合并成一个页面。KSM 可以通过设置不同的阈值来控制合并操作的频率和范围，同时也提供了接口供用户程序手动触发合并操作。
 * KSM 主要应用于虚拟化场景下的内存去重，如 KVM 虚拟机中的内存管理，以及一些需要大量内存的应用程序，如数据库系统等。
 ...
 * struct mm_slot 是一个用于管理进程地址空间（struct mm_struct）的数据结构，它被用于实现内核中的反向映射（reverse mapping）机制。
 * 具体来说，每个进程的地址空间可以被映射到多个物理页面，而反向映射机制维护了一张表，将物理页面映射到所有使用它的进程的虚拟地址。这张表的每个表项被称为 rmap_item，而 mm_slot 是对同一个进程的所有 rmap_item 的管理结构。
 */
struct mm_slot {
	// link 字段是一个哈希链表节点，用于将不同进程的 mm_slot 链接起来
	struct hlist_node link;
	// mm_list 则是一个 list_head，用于将同一个进程的所有 mm_slot 链接起来。
	struct list_head mm_list;
	// rmap_list 是一个指向当前 mm_slot 对应进程的 rmap_item 链表的指针
	struct rmap_item *rmap_list;
	// mm 指向了当前 mm_slot 所对应的进程的 mm_struct 数据结构。
	struct mm_struct *mm;
};
 ...
 * stable_node 表示 KSM 稳定树上的节点
 * rmap_item 会按照其虚拟地址的哈希值，被链接到对应的 stable_node 上，当需要对 KSM 进行扫描时，可以遍历整个稳定树，检查其各个节点中的 rmap_item，以查找是否有相同的页面可以合并。同时，在迁移过程中，stable_node 节点也会被加入到迁移列表中，用于协调迁移的操作。
 */
struct stable_node {
	union {
		// node：用于将 stable_node 节点插入到红黑树中；
		struct rb_node node;	/* when node of stable tree */
		// head、hlist_dup 和 list：当 stable_node 节点被列入到迁移列表时使用的成员，用于在双向链表和散列表中进行链接；
	...
	// hlist：散列表，用于将 rmap_item 按照虚拟地址哈希到 KSM 稳定树的节点上；
	struct hlist_head hlist;
	union {
		// kpfn：稳定节点对应的物理页帧号；
		unsigned long kpfn;
		// chain_prune_time：稳定节点最近一次使用时间；
...
// rmap_hlist_len：rmap_item 在该节点上的哈希表长度；
	int rmap_hlist_len;
#ifdef CONFIG_NUMA
// nid：numa 节点 ID。
	int nid;
 ...
 * 用于实现反向映射（reverse mapping）的数据结构rmap_item和mm_slot。
 * rmap_item是一个反向映射表项，表示一个页面和它在进程虚拟地址空间中的一个或多个映射关系。反向映射的作用是根据物理页面查找进程中与之相关联的虚拟地址，以便在页面回收等操作时进行清理。
 */
struct rmap_item {
	// rmap_list：用于链接所有映射到同一物理页面的rmap_item。
	struct rmap_item *rmap_list;
	union {
		// anon_vma：指向匿名VMA，表示该页面映射到了匿名内存区域。
		struct anon_vma *anon_vma;	/* when stable */
#ifdef CONFIG_NUMA
		// nid：表示该页面映射到了某个NUMA节点。
		int nid;		/* when node of unstable tree */
#endif
	};
	// mm：指向映射该页面的进程的mm_struct结构。
	struct mm_struct *mm;
	// address：表示该页面在虚拟地址空间中的一个映射地址。这个字段中低位用于存储一些标志位，如RMAP_IS_ANONYMOUS表示是否为匿名页面。
	unsigned long address;		/* + low bits used for flags below */
	// oldchecksum：表示该页面的checksum，用于检测页面内容是否发生变化。在rmap_item表示的映射关系稳定时使用。
	unsigned int oldchecksum;	/* when unstable */
	union {
		// node：用于将rmap_item插入进程的反向映射树中。在rmap_item表示的映射关系不稳定时使用。
		struct rb_node node;	/* when node of unstable tree */
		// head和hlist：用于将rmap_item插入待清理的列表中。在rmap_item表示的映射关系稳定时使用。
...
// KSM中的不稳定树和稳定树是两种不同的数据结构，它们分别用于管理不同阶段的页面合并。
// 在KSM的第一阶段中，不稳定树用于保存所有重复页面的信息。当KSM检测到两个或更多页面的内容相同时，它会将它们添加到不稳定树中，以便在下一阶段进行合并。在这个阶段中，页面的地址和内容都可能发生变化，因此这些页面被称为不稳定页面。
// 在KSM的第二阶段中，KSM会从不稳定树中选取一组合适的页面，并将它们合并成一个页面，并将这个新页面加入到稳定树中。在这个阶段中，页面的地址和内容已经固定下来，因此这些页面被称为稳定页面。
// 总之，不稳定树和稳定树是KSM内部使用的数据结构，用于管理KSM的不同阶段，以提高内存的使用效率。不稳定树用于保存重复页面的信息，稳定树用于管理合并后的页面。
